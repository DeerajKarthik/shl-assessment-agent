...
from app.catalog import canonicalize_url
...
def _schema_valid_json(payload: dict) -> bool:
    """Check hard-eval schema requirements on raw HTTP JSON."""
...
async def evaluate() -> dict[str, object]:
    ...
    traces = load_public_traces(Path("GenAI_HoldoutConversations"))
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        for trace in traces:
            ...
            for user_turn in trace.user_turns:
                ...
                # Rate limit delay BEFORE the request (not counted as latency)
                if service.gemini.enabled:
            ...
            if service.gemini.enabled:
                # Sleep for 15s between calls to stay under Groq 6000 TPM limit
